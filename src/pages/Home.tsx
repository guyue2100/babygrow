  ...
  return (
    <div className="min-h-screen pb-20 pt-6 md:pt-12 relative overflow-hidden bg-[#FDFBF7]">
      ...
      <div className="fixed top-4 right-4 z-50 flex items-center gap-4">
        <Link to="/articles" className="text-sm font-bold text-zinc-600 hover:text-indigo-600 bg-white/90 backdrop-blur px-5 py-2.5 rounded-full shadow-sm border border-zinc-200">Articles</Link>
        <LanguageSwitcher />
      </div>
      ...
      <main className="max-w-4xl mx-auto px-4 md:px-6 flex flex-col space-y-12">
        <section className="bg-white/40 backdrop-blur-md p-8 md:p-12 rounded-[2.5rem] border border-white/20 shadow-sm">
          <h2 className="text-2xl md:text-3xl font-black text-zinc-900 mb-6 tracking-tight leading-tight">{t('directAnswerTitle')}</h2>
          <p className="text-zinc-600 text-lg leading-relaxed"><strong className="text-zinc-900">{t('directAnswerBold')}</strong> {t('directAnswerText')}</p>
        </section>
        ...
        <section className="w-full">
          <HeightPredictor gender={assessmentData.gender} fatherHeight={assessmentData.fatherHeight} motherHeight={assessmentData.motherHeight} latestHeight={latestRecord?.height} latestAgeInMonths={latestRecord?.ageInMonths} />
        </section>

        <section className="bg-white p-8 md:p-12 rounded-[3rem] shadow-sm border border-black/5 space-y-12">
          <h3 className="text-2xl font-black text-zinc-900 flex items-center border-b pb-6"><TrendingUp className="w-8 h-8 mr-3 text-indigo-500" />{t('trends')}</h3>
              ...
              {recentArticles.map(article => (
                <Link key={article.id} to={`/articles/${article.id}`} className="bg-white p-7 rounded-[2.5rem] shadow-sm border border-zinc-100 hover:shadow-xl transition-all flex flex-col h-full">
                  ...
                  <div className="text-[10px] text-zinc-400 font-bold uppercase pt-5 border-t border-zinc-50 flex justify-between">
                    ...
                    <span className="flex items-center gap-1"><Eye className="w-3 h-3" />{800 + (article.read_count * 5)}</span>
                  </div>
                </Link>
              ))}
            </div>
